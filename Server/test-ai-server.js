...
async function testEndpoint(endpoint, data) {
	try {
		...
		if (result.success) {
			...
			console.log('Sample response:', JSON.stringify(result, null, 2).substring(0, 200) + '...');
		} else {
...
async function runTests() {
	...
	try {
		...
		console.log('❌ Server not running. Please start with: node ai-attack-analyzer.js');
		return;
	}
	...
	try {
		...
		const response = await fetch(`${SERVER_URL}/api/plan-attack-visual`, {
			...
			body: JSON.stringify({
				...
				availableArmy: "Giants: 8, Wizards: 16, Archers: 40, Wall Breakers: 6, Barbarian King, Archer Queen",
